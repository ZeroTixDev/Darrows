...
function updatePlayer(
    ...
    if (!player.dying) {
        ...
        let mag = Math.sqrt(dir.x * dir.x + dir.y * dir.y) || 1;
        // if (player.arrowing > 0) {
        //     dir.x = 0;
        //     dir.y = 0;
        //     mag = 1;
        ...
        if (player.character.Ability != null && !isClone) {
            ...
            if (player.character.Ability.name === 'Freeze-Arrow') {
                ...
                for (const arrow of Object.values(arrows)) {
                    ...
                    if (
                        (!input.shift && arrow.freezed) ||
                        (arrow.freezed &&
                            player.timeSpentFreezing >=
                                player.timeFreezeLimit) ||
                        (arrow.freezed && arrow.dead && arrow.life <= dt)
                        ...
                        player.abilityCooldown =
                            0.5 + player.timeSpentFreezing * 0.8;
                        player.maxCd = player.abilityCooldown;
            ...
            if (player.character.Ability.name === 'Gravity') {
                ...
                if (player.usingGravity) {
                    for (const arrowId of Object.keys(arrows)) {
                        if (
                            arrows[arrowId].parent === player.id &&
                            arrows[arrowId].gravity
                        ) {
                ...
                if (
                    (!input.shift && !player.passive && player.usingGravity) ||
                    (player.usingGravity &&
                        !player.passive &&
                        !hasGravityArrow &&
            ...
            if (player.character.Ability.name === 'Dash') {
                player.canDash =
                    !player.passive &&
                    player.abilityCooldown <= 0 &&
                    (player.arrowing > 0 || player.lastDashForce != null);
                let should = true;
                if (
                    player.arrowing > 0 &&
                    player.abilityCooldown <= 0 &&
                    !player.changedLastTime
                ) {
                ...
                if (player.canDash && input.shift && should) {
                    ...
                    player.bxv =
                        Math.cos(player.dashAngle) * 1100 +
                        Math.cos(player.dashAngle) * force * 5750;
                    player.byv =
                        Math.sin(player.dashAngle) * 1100 +
                        Math.sin(player.dashAngle) * force * 5750;
                }
            }
            ...
            if (player.character.Ability.name === 'Flank-Around') {
                if (
                    ...
                    for (const playerId of Object.keys(players)) {
                        ...
                        if (
                            other.timer > 0 ||
                            (other.character.Name === 'Scry' && !other.showAim)
                        ) {
                            continue;
                        }
                        const distX = player.x - other.x;
                    ...
                    if (otherId != null) {
                        ...
                        player.y = dest.y;
                        while (
                            player.intersectingObstacles(obstacles) &&
                            teleportDist > 0
                        ) {
                            teleportDist -= 10;
                            dest = {
                                x:
                                    other.x +
                                    Math.cos(player.angle) * teleportDist,
                                y:
                                    other.y +
                                    Math.sin(player.angle) * teleportDist,
                            };
                            player.x = dest.x;
                            player.y = dest.y;
                        }
            ...
            if (player.character.Ability.name === 'Direct-Arrow') {
                if (
                    ...
                    player.redirPoint = {
                        x:
                            player.x +
                            Math.cos(player.angle) * (player.radius + 250),
                        y:
                            player.y +
                            Math.sin(player.angle) * (player.radius + 250),
                    };
                    player.abilityCooldown = 8;
                    player.maxCd = 8;
                }
                ...
                if (
                    ...
                    player.arrowing <= 0 &&
                    input.shift &&
                    player.redirPoint != null &&
                    player.canRedirect
                ) {
                    let newestArrow = null;
                    for (const arrow of Object.values(arrows)) {
                        if (arrow.parent != player.id || arrow.redirected)
                            continue;
                        if (newestArrow == null || arrow.c < newestArrow.c) {
                    ...
                    if (newestArrow != null) {
                        ...
                        newestArrow.angle = Math.atan2(
                            player.redirPoint.y - newestArrow.y,
                            player.redirPoint.x - newestArrow.x
                        );
				...
				if (player.canRedirectAgain && !input.shift && !player.passive
				   && player.arrowing <= 0 && !player.canRedirect) {
					player.canRedirectAgain = null;
					player.redirPoint = null;
				}
        ...
        if (!changedMovement) {
            player.xv +=
                normal.x *
                ((player.arrowing > 0 ? player.speed * 0.75 : player.speed) *
                    dt);
            player.yv +=
                normal.y *
                ((player.arrowing > 0 ? player.speed * 0.75 : player.speed) *
        ...
        if (!player.passive) {
            if (input.space && player.timer <= 0) {
                ...
                if (player.arrowing > 0 && player.timer <= 0) {
                    ...
                    // }
                    if (
                        player.character.Ability != null &&
                        player.character.Ability.name === 'Gravity' &&
                        player.usingGravity
                    ) {
                    ...
                    if (
                        player.character.Ability != null &&
                        player.character.Ability.name === 'Fake-Arrow'
                    ) {
                        player.noAim = 0;
                    ...
                    if (
                        player.character.Ability != null &&
                        player.character.Ability.name === 'Dash'
                    ) {
        ...
        let fric = player.fric;
        player.xv *= Math.pow(fric, dt * 60);
        player.yv *= Math.pow(fric, dt * 60);
...
function boundPlayerObstacle(player, obstacle) {
    ...
    if (
        distX < rectHalfSizeX + player.radius &&
        distY < rectHalfSizeY + player.radius
    ) {
...
function collidePlayers(players, arena, obstacles) {
    ...
    for (let i = 0; i < keys.length; i++) {
        const player1 =
            typeof keys[i] === 'object' && keys[i].clone
                ? players[keys[i].parent].clones[keys[i].index]
                : players[keys[i]];
        for (let j = 0; j < keys.length; j++) {
            if (i >= j) continue;
            const player2 =
                typeof keys[j] === 'object' && keys[j].clone
            ...
            if (
                ...
                // player2.yv -= yv * 10;
                player1.x =
                    player2.x + (player1.radius + 0.05 + player2.radius) * xv;
                player1.y =
                    player2.y + (player1.radius + 0.05 + player2.radius) * yv;
                player2.x =
                    oldP.x - (player1.radius + 0.05 + player2.radius) * xv;
                player2.y =
                    oldP.y - (player1.radius + 0.05 + player2.radius) * yv;
    ...
    for (const playerId of Object.keys(players)) {
        ...
        if (
            distX * distX + distY * distY <
            player.radius * 2 * (player.droneRadius * 2)
        ) {
            const magnitude = Math.sqrt(distX * distX + distY * distY) || 1;
            const xv = distX / magnitude;
            const yv = distY / magnitude;
        ...
        for (const otherId of Object.keys(players)) {
            ...
            if (
                distX * distX + distY * distY <
                other.radius * 2 * (player.droneRadius * 2)
            ) {
                const magnitude = Math.sqrt(distX * distX + distY * distY) || 1;
                const xv = distX / magnitude;
                const yv = distY / magnitude;
                ...
                // player.y = player.droneY + ((player.radius + 0.05 + player.droneRadius) * (yv));
                player.droneX =
                    oldP.x - (other.radius + player.droneRadius) * xv;
                player.droneY =
                    oldP.y - (other.radius + player.droneRadius) * yv;
            }
        }
        const dronePlayer = {
            x: player.droneX,
            y: player.droneY,
            radius: player.droneRadius,
            xv: 0,
            yv: 0,
            score: 0,
        };
